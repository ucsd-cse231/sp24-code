...
fn expr_vars(e: &Expr) -> usize {
    match e {
        ...
        | Expr::Le(e1, e2)
        | Expr::Plus(e1, e2)
        | Expr::Mult(e1, e2) => max(expr_vars(e1), 1 + expr_vars(e2)),
...
fn compile_def_body(
    ...
    let body_code = compile_expr(body, &init_env(args), sp, count, &exit_label, true, f);
...
fn compile_def(def: &Defn, count: &mut i32) -> String {
    ...
    let body = compile_def_body(fun_name, &args, 1, body, count);
    format!(
        "fun_start_{fun_name}:
            {body}"
    )
